...
There are two basic categories of tactics:
1. **Solutions:** actually fixing the flaky tests
2. **Mitigations:** minimising harm when the flaky tests cannot be fixed
...
A fundamental idea in software testing is to reproduce a bug.

As mentioned previously, flaky tests are difficult to reproduce consistently because their failure is non-deterministic: sometimes they fail sometimes not.
...
Additionally, we could simulate failure conditions, to increase the failure rate for reproduction purposes.
...
> Aside: More on 3.: by running a higher number of workers than CPU cores, we effectively add more workload to our machine, causing it to run slower. This can help create conditions that will reproduce a test's flakiness.
...
Problem: Jest allows the maximum number of workers to be configured via the [`maxWorkers`](https://jestjs.io/docs/configuration#maxworkers-number--string) setting. However, if this exceeds the number of CPU cores on the machine, the processor may be forced to split execution time between multiple threads. This may cause tests to take longer than expected to execute, resulting in timeouts. This problem may only occur on CI environments, where CPU resources may be more limited, making it tricky to identify.

Solution: Check the CPU configuration of the CI environment if possible. Try reducing the `maxWorkers` setting or even eliminating it. (Jest defaults to the number of cores available on the machine, which is usually the safest bet.)
...
In Jest, a similar effect can be achieved by regex filtering the test name using the [testNamePattern](https://jestjs.io/docs/cli#--testnamepatternregex) config setting:
...
❌
```javascript
  expect(timeAfterClickPause.getTime()).toEqual(new Date(2026, 3, 1, 13, 1, 1).getTime());
```
