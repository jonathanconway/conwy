...
> Aside: **Warning:** It's important at this point to note that *incorrect function* of a test can occur in various non-obvious ways ways, from giving false-positives (passing when it should fail) to ending prematurely (i.e. ending before the system functionality has been fully exercised).
...
```bash
npx playwright test ≪test-path-filename≫ --repeat-each=20 --fail-on-flaky-tests
```
...
- https://www.codewithjason.com/how-i-fix-flaky-tests/
