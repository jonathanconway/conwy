...
Pull requests are _the_ fundamental unit of change on many software projects. They allow code to be reviewed before going to Production and later serve as a handy reference when tracing the history of the code base.

Previously I wrote some of my best tips on [code reviews](/articles/code-reviews) of others' pull requests. In this article I'll cover my learnings to date about what makes a good pull request.


## Cohesion over size

Though there are debates about pull request size, I think size is less important than _shape_.
...
I find the concept of "unit of change" useful here. If a change can be mentally treated as a single cohesive unit, then it's easier to reason about and review. This is based on the concepts of [chunking in Psychology](https://en.wikipedia.org/wiki/Chunking_(psychology)) and [Single-responsibility principle (SRP)](https://en.wikipedia.org/wiki/Single-responsibility_principle) in programming.

Applying this concept to pull requests:
...
However, sometimes / occasionally it makes sense to split related changes across multiple pull requests. Some changes lend themselves to being decomposed into a sequence of smaller changes. For example, removing a feature from an application might impact multiple unrelated areas of the application. In that case, it might make sense to remove the feature from each area one at a time, until it has been completely removed. At each stage of the removal, the whole application should build and function properly.
...
Have you ever been faced with a massive pull request – thousands of lines – only to discover on investigation that it's just a formatting change? For example, it might be a change from tabs to white spaces or from TSV to CSV.

If possible, it's better to perform such a formatting change in a separate pull request, clearly labelled as such. This will save reviewers a lot of time spent investigating why the change is so large.
...
Sometimes this separation isn't easy or even possible, however. For example, automated formatting tools might force formatting changes to be included in content changes.

For these cases, I try to add a brief [inline note](#inline-notes) to each block of changes, informing the reviewer that they are just formatting changes.
...
For example, suppose we need to upgrade a core dependency, say NodeJS, which has multiple second-order dependencies. All of the dependencies need to be upgraded together. But it is impossible or unadvisable to upgrade each second-order dependency in a completely separate pull request. In cases like these, I found the approach of "sub pull requests" useful. Here we create one large pull request in the main branch.
...
{/* todo: when and how to split chunks */}
{/* todo: avoid too many chunks*/}
{/* todo:  */}
...
{/* todo: link to "right balance" article */}
...
Some content I might include in a description, if not already covered elsewhere:

- **Links.** To task ticket and other pull requests (such as parts of a sequence).
- **Summary.** Of changes.
- **Technical details.** Of changes.
- **Local setup instructions.** For example: shell commands, port numbers, environment variables, feature flags, etc.
...
- **Screenshots.** Before and after and various screen sizes. Recordings.
...
I find it helpful to keep a handful of "snippets" – or pieces of Markdown that I can copy and paste to quickly scaffold a pull request description.

These can be kept as, say, Markdown files in a local folder.

For example, if my pull requests frequently include "feature flag" instructions, I'll set up a reusable "feature flag" snippet. Whenever I need to, I can copy and paste the "feature flag" snippet into the pull request and then easily adjust the feature flag keys.
...
I like to prefix the notes to make them easier for code reviewers to scan.
...
- `Testing:` - instructions on how to test a piece of code.

I include testing instructions with small snippets of code, if I think it will save the reviewer's time, and assuming they cannot be expressed in an automated test.
...
If there are multiple inline notes, I list them, along with links, in the description, in a collapsible region. This allows the reviewer to glance and scan them at a high level.
...
For this reason I often adopt a policy of taking deeper discussions into a more appropriate forum, such as team chat (Slack or equivalent) or a call (Teams, Google Meet or equivalent). Team chat threads or call recordings can then be linked into the pull request, either in the comments thread or the description area. These can serve as valuable references for future maintainers.
...
Submitting pull requests in draft mode can be a good way to "sketch" a change and get early feedback on the overall direction before going too deep. I've found draft pull requests a great way to do solution design as a team, as it grounds the design **in the code** rather than getting too abstract with diagrams or wordy descriptions.
