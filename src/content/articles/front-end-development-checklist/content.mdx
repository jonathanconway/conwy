...
With a checklist of 575 items and growing, you might wonder how I have the time to go through that many items. The truth is that I only use a subset of checklist items per situation. The checklist items are grouped into headings and tags which can be filtered. Using the filters, I can quickly narrow down the list to a much more manageable size.

For example, when reviewing a code change that only affects styling, I can filter by the `CSS`, `SCSS and LESS` and `Accessibility/Appearance` tags.

![Screenshot of filters with relevant tags selected](/images/articles/front-end-development-checklist/styling-filters-screenshot.png)
