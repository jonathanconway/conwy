...
In this article I'll outline motivate and outline the pattern, including my own `createWithHOC` implementation and a Tooltip example.
...
Here's an example of multiple different kinds of controls in a form, utilizing the generic HOC pattern.

Each passes a prop for one or more of the following generic components:
- Label
- Validation
- Default
...
As a consumer, it would be easy to determine whether a given component supports the tooltip or not – we could simply examine the available pops via our IDE's autosuggest feature.

Here's an example in Visual Studio Code:

![Looking up the `tooltip` prop on an `IconButton` component in the conwy.co code-base](/images/articles/react-generic-hoc-pattern/vs-code-intellisense-example.png)
